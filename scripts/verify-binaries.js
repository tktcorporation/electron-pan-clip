...
const warningBinaries = [];

// NPMディレクトリが存在するか確認
if (!fs.existsSync(npmDir)) {
	console.error(`エラー: "${npmDir}" ディレクトリが見つかりません。`);
	console.error(
		"napi prepublish コマンドが正常に実行されていない可能性があります。",
	);
	exit(1);
}
...
for (const binary of platformsToCheck) {
	...
	if (!fs.existsSync(binaryPath)) {
		...
		if (fileSizeInKB < 10) {
			console.warn(
				`警告: "${binary}" のサイズが小さすぎます (${fileSizeInKB}KB)。正常にビルドされていない可能性があります。`,
			);
			if (!isOptional) {
				missingBinaries.push(binary);
			}
...
if (missingBinaries.length > 0) {
	...
	console.error(
		'package.jsonの"napi.triples"セクションを確認し、すべての必要なプラットフォームが含まれていることを確認してください。',
	);
	exit(1);
} else if (warningBinaries.length > 0) {
	console.warn("\n⚠️ 以下のオプショナルバイナリファイルが見つかりません:");
	for (const binary of warningBinaries) {
		console.warn(`- ${binary}`);
	}
	console.warn(
		"これらは一部の環境で構築が難しい場合があるため、オプショナルとして扱われます。",
	);
	console.log("\n✅ 必須バイナリはすべて存在します。");
} else {
